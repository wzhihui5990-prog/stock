# -*- coding: utf-8 -*-
"""
SPY 末日期权双买策略回测 + HTML 可视化
数据来源：
  - SPY 股价：1-spy日K/data/spy_market_data.xlsx
  - 期权数据：2-spy末日期权…/data/spy_0dte_options_offset3.xlsx

策略规则：
  T-1 收盘：同时买入 Call(+3) 和 Put(-3)，各1张，成本 = Call_T2收盘 + Put_T2收盘
  T（到期日）9:30~12:00：若 SPY 涨跌幅超过 ±阈值%（基于T-1收盘价），立即卖出双腿
...
# 配置
# ────────────────────────────────────────────────
SPY_FILE    = os.path.join(os.path.dirname(__file__), "..", "1-spy日K", "data", "spy_market_data.xlsx")
OPT_FILE_3  = os.path.join(os.path.dirname(__file__), "data", "spy_0dte_options_offset3.xlsx")
OPT_FILE_4  = os.path.join(os.path.dirname(__file__), "..", "3-spy末日期权日K-上下4股价的期权合同-前一天末日期权的收盘价", "data", "spy_0dte_options_offset4.xlsx")
OUTPUT_HTML = os.path.join(os.path.dirname(__file__), "data", "spy_0dte_strategy_report.html")

UPPER_TRIGGER_PCT = 0.75
LOWER_TRIGGER_PCT = 0.75
COMMISSION   = 1.7
MONITOR_START = "09:30"
MONITOR_END   = "10:30"
...
def load_data(opt_file):
    ...
    spy_1m    = pd.read_excel(SPY_FILE, sheet_name="SPY_分时1min")
    spy_2m    = pd.read_excel(SPY_FILE, sheet_name="SPY_分时2min")
    spy_5m    = pd.read_excel(SPY_FILE, sheet_name="SPY_5min")
    spy_daily = pd.read_excel(SPY_FILE, sheet_name="SPY_日K")
    return summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m, spy_daily


def _get_spy_day(t1, spy_1m, spy_2m, spy_5m):
    """按 1min→2min→5min 降级获取当天 SPY 数据，返回 (df, granularity)"""
    for df, label in [(spy_1m, "1min"), (spy_2m, "2min"), (spy_5m, "5min")]:
...
def run_backtest(summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m):
    results = []
    ...
    for _, row in summary.iterrows():
        ...
        spy_day, granularity = _get_spy_day(t1, spy_1m, spy_2m, spy_5m)
        if spy_day.empty:
            continue

        spy_t2_close = float(row["SPY_T2收盘"])

        spy_open_row = spy_day[spy_day["time_only"] == "09:30"]
        if spy_open_row.empty:
            spy_open_row = spy_day.iloc[:1]
        spy_open = float(spy_open_row.iloc[0]["收盘价"])
        ...
        monitor_spy = spy_day[(spy_day["time_only"] >= MONITOR_START) & (spy_day["time_only"] <= MONITOR_END)]

        trigger_time = None
        trigger_pct  = None
        trigger_dir  = None

        for _, mrow in monitor_spy.iterrows():
            t = mrow["time_only"]
            price = float(mrow["收盘价"])
            pct = (price - spy_t2_close) / spy_t2_close * 100
        ...
        spy_close_row = spy_day.iloc[-1]
        spy_close = float(spy_close_row["收盘价"])
        spy_day_pct = round((spy_close - spy_t2_close) / spy_t2_close * 100, 2)

        results.append({
            "到期日": t1,
            "基准日": t2,
            "SPY_T2收盘": spy_t2_close,
            "SPY开盘": spy_open,
            "SPY收盘": spy_close,
            "SPY涨跌%": spy_day_pct,
            "Call成本": call_cost,
...
def build_daily_charts(results, call_1m, put_1m, spy_1m, spy_2m, spy_5m):
    daily_data = []

    for r in results:
        t1 = r["到期日"]

        spy_day, _ = _get_spy_day(t1, spy_1m, spy_2m, spy_5m)
        spy_arr = []
        for _, row in spy_day.iterrows():
            t = str(row["时间"])[-5:]
            spy_arr.append({
                "t": t,
                "o": float(row["开盘价"]),
...
<div class="header">
  ...
  <div class="sub-detail">
    ...
    <div class="item"><span class="tag green">触发卖出</span>T（到期日当天）美东 9:35 起监控，SPY 相对 T-1 收盘价涨幅 ≥ +阈值% 或跌幅 ≥ −阈值%，立即同时卖出 Call + Put</div>
    <div class="item"><span class="tag red">止损</span>若监控区间全程未触发，于平仓时间以当时价止损平仓，通常大幅亏损</div>
...
<div class="section">
  ...
  <table id="tradeTable">
    <thead>
      <tr>
        <th>到期日</th><th>粒度</th><th>T-1收盘</th><th>T开盘</th><th>SPY涨跌%(vs T-1收)</th>
...
"""
    ...
    for i, r in enumerate(results):
        ...
        qqq_pct_class = "pnl-pos" if r["SPY涨跌%"] > 0 else "pnl-neg"
        ...
        <td>${r["SPY_T2收盘"]}</td>
        <td>${r["SPY开盘"]}</td>
        <td class="{qqq_pct_class}">{r["SPY涨跌%"]}%</td>
...
function _rebuildTableRows(results) {
  ...
  results.forEach((r, i) => {
    ...
    const qpc = r['SPY涨跌%'] >= 0 ? 'pnl-pos' : 'pnl-neg';
...
function selectDay(idx) {
    ...
    <td colspan="17">
      <div class="detail-inner">
        ...
        <div class="chart-grid3">
          <div class="chart-box">
            <div class="chart-box-title">SPY 走势（${d.granularity}）</div>
            <canvas id="spyCanvas" style="width:100%;height:340px;display:block;"></canvas>
          </div>
          <div class="chart-box">
  ...
  requestAnimationFrame(() => {
    ...
    const t2Close   = r["SPY_T2收盘"];
    const spyOpen   = r["SPY开盘"];
    const upper     = t2Close * (1 + _upperPct/100);
    const lower     = t2Close * (1 - _lowerPct/100);

    drawCandleChart('spyCanvas', d.spy,
      ...
      [{val:upper,   color:'#3fb950', label:'+'+_upperPct+'%'},
       ...
       {val:spyOpen, color:'#8b949e', label:'T开$'+spyOpen.toFixed(2)}],
...
def main():
    ...
    print("加载数据...")
    summary3, call3_1m, put3_1m, spy_1m, spy_2m, spy_5m, spy_daily = load_data(OPT_FILE_3)
    summary4, call4_1m, put4_1m, _,      _,      _,      _          = load_data(opt_file_4)

    print("运行策略回测（±3）...")
    results3 = run_backtest(summary3, call3_1m, put3_1m, spy_1m, spy_2m, spy_5m)
    print(f"  ±3 共 {len(results3)} 个交易日")
    for g, cnt in pd.Series([r['数据粒度'] for r in results3]).value_counts().items():
        print(f"    {g}: {cnt} 天")

    print("运行策略回测（±4）...")
    results4 = run_backtest(summary4, call4_1m, put4_1m, spy_1m, spy_2m, spy_5m)
    print(f"  ±4 共 {len(results4)} 个交易日")
    for g, cnt in pd.Series([r['数据粒度'] for r in results4]).value_counts().items():
        print(f"    {g}: {cnt} 天")

    print("构建日内图表数据...")
    daily3 = build_daily_charts(results3, call3_1m, put3_1m, spy_1m, spy_2m, spy_5m)
    daily4 = build_daily_charts(results4, call4_1m, put4_1m, spy_1m, spy_2m, spy_5m)

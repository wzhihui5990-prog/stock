# -*- coding: utf-8 -*-
"""
QQQ 末日期权（0DTE）历史数据批量拉取（增量追加模式）
基于 QQQ 当天开盘价 ± 3 确定行权价
...
END_DATE      = None
OUTPUT_DIR    = "data"
OUTPUT_FILE   = os.path.join(OUTPUT_DIR, "qqq_0dte_options_open_offset3.xlsx")
...
def get_qqq_daily_bars(start, end):
    """获取 QQQ 日K，包含开盘价和收盘价"""
    ...
    for r in data.get("results", []):
        ...
        days.append({
            ...
            "close": round(r.get("c", 0), 2),
        })
    return days


def build_ticker(expiration, option_type, strike):
...
def fetch_daily_bar(ticker, day):
...
def fetch_1min_bars(ticker, day):
    url  = f"{BASE_URL}/v2/aggs/ticker/{ticker}/range/1/minute/{day}/{day}"
    data = api_get(url, {"adjusted": "true", "sort": "asc", "limit": 1000})
    rows = []
    for r in data.get("results", []):
        dt_et = datetime.fromtimestamp(r["t"] / 1000, tz=timezone.utc).astimezone(EASTERN).strftime("%Y-%m-%d %H:%M")
        rows.append({
            "到期日":     day,
            "时间(美东)": dt_et,
            "开盘价":     round(r.get("o", 0), 4),
...
def process_day(t2_date, t2_close, t1_date, t1_open):
    """
    基于 QQQ 当天(T1)开盘价确定行权价
    t1_open: QQQ 在到期日(T1)的开盘价
    """
    base_strike = round(t1_open)
    ...
    print(f"  T1={t1_date} QQQ开盘=${t1_open}  T2={t2_date} QQQ收盘=${t2_close}")
    print(f"    行权价基准=round({t1_open})={base_strike}  Call=${call_strike} {call_ticker}  Put=${put_strike} {put_ticker}")

    # T2(前一天)的期权日K — 用于获取 T-1 收盘价
    ...
    print(f"    Call: T2收盘={'$'+str(call_t2['收盘价']) if call_t2 else '无'}  T1开盘={'$'+str(call_t1['开盘价']) if call_t1 else '无'}  T1收盘={'$'+str(call_t1['收盘价']) if call_t1 else '无'}  1min={len(call_1m)}条")
    print(f"    Put : T2收盘={'$'+str(put_t2['收盘价'])  if put_t2  else '无'}  T1开盘={'$'+str(put_t1['开盘价']) if put_t1 else '无'}  T1收盘={'$'+str(put_t1['收盘价'])  if put_t1  else '无'}  1min={len(put_1m)}条")
...
def write_excel(df_summary, df_call_1m, df_put_1m):
    with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
        for sheet_name, df in [("摘要", df_summary), ("Call_1min", df_call_1m), ("Put_1min", df_put_1m)]:
...
def main():
    ...
    qqq_days = get_qqq_daily_bars(fetch_start, end_date)
    if len(qqq_days) < 2:
        print("获取交易日失败，请检查 API Key")
        return
    print(f"  共 {len(qqq_days)} 个交易日")

    # 构建 T1 开盘价查找表
    open_map = {d["date"]: d["open"] for d in qqq_days}

    pairs_to_process = []
    for i in range(len(qqq_days) - 1):
        t2      = qqq_days[i]
        t1_date = qqq_days[i + 1]["date"]
        t1_open = qqq_days[i + 1]["open"]
        t1_d    = datetime.strptime(t1_date, "%Y-%m-%d").date()
        if t1_d >= START_DATE and t1_date not in existing_dates and t1_d < end_date:
            pairs_to_process.append((t2, t1_date, t1_open))
    ...
    for t2, t1_date, t1_open in pairs_to_process:
        print(f"── {t1_date} ──")
        result = process_day(t2["date"], t2["close"], t1_date, t1_open)
    ...
    def merge(old, new_rows, dedup_cols):
        ...
        return combined.drop_duplicates(subset=dedup_cols).sort_values(dedup_cols).reset_index(drop=True)
    ...
    if all(r["Call_1min条数"] == 0 and r["Put_1min条数"] == 0 for r in new_summaries):
        print("\n  1分钟数据均为空 — Polygon 免费账号不支持期权分钟级历史数据，需升级 Starter 套餐")

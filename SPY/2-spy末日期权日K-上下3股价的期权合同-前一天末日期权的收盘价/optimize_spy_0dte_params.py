...
def load_data(opt_file):
    ...
    return summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m


def _get_spy_day(t1, spy_1m, spy_2m, spy_5m):
    for df in [spy_1m, spy_2m, spy_5m]:
...
def build_daily_records(summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m):
    ...
    for _, row in summary.iterrows():
        ...
        spy_day = _get_spy_day(t1, spy_1m, spy_2m, spy_5m)
        if spy_day.empty:
            continue
        ...
        # 只取 9:30 之后的数据
        spy_monitor = spy_day[spy_day["time_only"] >= MONITOR_START][["time_only", "收盘价"]].values.tolist()
...
def main():
    ...
    for label, opt_file in STRIKE_FILES:
        print(f"  处理 {label}...")
        summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m = load_data(opt_file)
        records = build_daily_records(summary, call_1m, put_1m, spy_1m, spy_2m, spy_5m)
        n_days = len(records)
    ...
    # 保存完整结果
    out = os.path.join(os.path.dirname(__file__), "data", "spy_0dte_param_optimization.csv")

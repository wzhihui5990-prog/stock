...
def precompute(summary, call_1m, qqq_1m, qqq_2m, qqq_5m):
    """预计算每天的时间序列，避免重复过滤"""
    days = []
    for _, row in summary.iterrows():
        ...
        call_prices = {}
        for _, cr in c1m.iterrows():
            call_prices[cr["time_only"]] = float(cr["收盘价"])
...
def run_single(days, upper_pct, lower_pct, close_time):
    ...
    for d in days:
        ...
        call_sell = d["call_prices"].get(sell_time)
        if call_sell is None:
            # 最近的可用价格
            best_t = None
            for ct in sorted(d["call_prices"].keys()):
                if ct <= sell_time:
                    best_t = ct
            call_sell = d["call_prices"][best_t] if best_t else 0
...
def optimize(opt_file, label):
    ...
    days = precompute(summary, call_1m, qqq_1m, qqq_2m, qqq_5m)

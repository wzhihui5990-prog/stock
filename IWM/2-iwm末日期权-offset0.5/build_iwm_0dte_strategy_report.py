# -*- coding: utf-8 -*-
"""
IWM 末日期权双买策略回测 + HTML 可视化
数据来源：
  - IWM 股价：1-iwm日K/data/iwm_market_data.xlsx
  - 期权数据：2-iwm末日期权-offset0.5/data/iwm_0dte_options_offset05.xlsx
             3-iwm末日期权-offset1/data/iwm_0dte_options_offset1.xlsx

策略规则：
  T-1 收盘：同时买入 Call(向上取整) 和 Put(向下取整)，各1张，成本 = Call_T2收盘 + Put_T2收盘
  T（到期日）9:30~12:00：若 IWM 涨跌幅超过 ±1.5%（基于T-1收盘价），立即卖出双腿
...
OUTPUT_HTML = os.path.join(os.path.dirname(__file__), "data", "iwm_0dte_strategy_report.html")

UPPER_TRIGGER_PCT = 3.0    # 上涨触发阈值 +3.0%
LOWER_TRIGGER_PCT = 0.5    # 下跌触发阈值 -0.5%
...
def load_data(opt_file):
    ...
    iwm_daily = pd.read_excel(IWM_FILE, sheet_name="IWM_日K")
    return summary, call_1m, put_1m, iwm_1m, iwm_2m, iwm_5m, iwm_daily


def _get_iwm_day(t1, iwm_1m, iwm_2m, iwm_5m):
    """按 1min→2min→5min 降级获取当天 IWM 数据，返回 (df, granularity)"""
    for df, label in [(iwm_1m, "1min"), (iwm_2m, "2min"), (iwm_5m, "5min")]:
...
def run_backtest(summary, call_1m, put_1m, iwm_1m, iwm_2m, iwm_5m):
    results = []
    ...
    for _, row in summary.iterrows():
        ...
        # T-1 当天的 IWM 数据（1min → 2min → 5min 降级）
        iwm_day, granularity = _get_iwm_day(t1, iwm_1m, iwm_2m, iwm_5m)
        if iwm_day.empty:
            continue
        ...
        # IWM T-1 开盘价（9:30 的收盘价，用于展示）
        iwm_open_row = iwm_day[iwm_day["time_only"] == "09:30"]
        if iwm_open_row.empty:
            iwm_open_row = iwm_day.iloc[:1]
        iwm_open = float(iwm_open_row.iloc[0]["收盘价"])
        ...
        # 监控区间
        monitor_iwm = iwm_day[(iwm_day["time_only"] >= MONITOR_START) & (iwm_day["time_only"] <= MONITOR_END)]

        trigger_time = None
        trigger_pct  = None
        trigger_dir  = None

        for _, mrow in monitor_iwm.iterrows():
            t = mrow["time_only"]
            price = float(mrow["收盘价"])
            pct = (price - iwm_t2_close) / iwm_t2_close * 100
        ...
        # IWM 当日涨跌幅（相对 T-1 收盘价）
        iwm_close_row = iwm_day.iloc[-1]
        iwm_close = float(iwm_close_row["收盘价"])
        iwm_day_pct = round((iwm_close - iwm_t2_close) / iwm_t2_close * 100, 2)

        results.append({
            "到期日": t1,
            "基准日": t2,
            "IWM_T2收盘": iwm_t2_close,
            "IWM开盘": iwm_open,
            "IWM收盘": iwm_close,
            "IWM涨跌%": iwm_day_pct,
            "Call成本": call_cost,
...
def build_daily_charts(results, call_1m, put_1m, iwm_1m, iwm_2m, iwm_5m):
    """为每个交易日构建 IWM / Call / Put 的数据 + 标记点"""
    daily_data = []
    ...
    for r in results:
        ...
        # IWM（使用与回测相同粒度）
        iwm_day, _ = _get_iwm_day(t1, iwm_1m, iwm_2m, iwm_5m)
        iwm_arr = []
        for _, row in iwm_day.iterrows():
            t = str(row["时间"])[-5:]
            iwm_arr.append({
                "t": t,
                "o": float(row["开盘价"]),
...
<body>
<div class="header">
  <h1>IWM (罗素2000) 末日期权双买策略回测分析</h1>
  <div class="sub">双买 0DTE 期权策略回测 &nbsp;·&nbsp; 数据范围：{results_05[0]["到期日"]} ~ {results_05[-1]["到期日"]} &nbsp;·&nbsp; 共 {total_trades} 个交易日</div>
  <div class="sub-detail">
    <div class="item"><span class="tag blue">建仓 T-1</span>以 T-1（到期日前一天）收盘价买入 Call(行权价向上取整或+1) + Put(行权价向下取整或-1) 各一张</div>
    <div class="item"><span class="tag green">触发卖出</span>T（到期日当天）美东 9:35 起监控，IWM 相对 T-1 收盘价涨幅 ≥ +阈值% 或跌幅 ≥ −阈值%，立即同时卖出 Call + Put</div>
...
<div class="section">
  ...
  <table id="tradeTable">
    <thead>
      <tr>
        <th>到期日</th><th>粒度</th><th>T-1收盘</th><th>T开盘</th><th>IWM涨跌%(vs T-1收)</th>
...
"""
    ...
    for i, r in enumerate(results):
        ...
        iwm_pct_class = "pnl-pos" if r["IWM涨跌%"] > 0 else "pnl-neg"
        ...
        <td>${r["IWM_T2收盘"]}</td>
        <td>${r["IWM开盘"]}</td>
        <td class="{iwm_pct_class}">{r["IWM涨跌%"]}%</td>
...
<script>
"""
    html += f"const RESULTS_05  = {json.dumps(results_05, ensure_ascii=False)};\n"
    html += f"const DAILY_05    = {json.dumps(daily_05, ensure_ascii=False)};\n"
    html += f"const RESULTS_1   = {json.dumps(results_1, ensure_ascii=False)};\n"
    html += f"const DAILY_1     = {json.dumps(daily_1, ensure_ascii=False)};\n"
    # 初始盈亏序列
    cum_05 = []
    s = 0
    for r in results_05:
        s += r['盈亏']
        cum_05.append(round(s, 4))
    cum_1 = []
    s = 0
    for r in results_1:
        s += r['盈亏']
        cum_1.append(round(s, 4))
    html += f"const CUM_PNL_05  = {json.dumps(cum_05)};\n"
    html += f"const CUM_PNL_1   = {json.dumps(cum_1)};\n"
...
function switchStrike(n) {
  _strike = n;
  _baseResults = n === 0.5 ? RESULTS_05.slice() : RESULTS_1.slice();
  _baseDaily   = n === 0.5 ? DAILY_05 : DAILY_1;
  document.getElementById('btn-strike05').classList.toggle('active', n === 0.5);
  document.getElementById('btn-strike1').classList.toggle('active', n === 1);
  // 关闭展开行
  if (currentIdx >= 0) {
...
function applyThreshold() {
  ...
  for (let i = 0; i < _baseResults.length; i++) {
    ...
    const commissionTotal = _commission * 4 / 100;
...
function _rebuildTableRows(results) {
  ...
  results.forEach((r, i) => {
    ...
    const qpc = r['IWM涨跌%'] >= 0 ? 'pnl-pos' : 'pnl-neg';
...
function selectDay(idx) {
    ...
    <td colspan="17">
      <div class="detail-inner">
        ...
        <div class="chart-grid3">
          <div class="chart-box">
            <div class="chart-box-title">IWM 走势（${d.granularity}）</div>
            <canvas id="iwmCanvas" style="width:100%;height:340px;display:block;"></canvas>
          </div>
          <div class="chart-box">
  ...
  requestAnimationFrame(() => {
    ...
    const t2Close   = r["IWM_T2收盘"];
    const iwmOpen   = r["IWM开盘"];
    const upper     = t2Close * (1 + _upperPct/100);
    const lower     = t2Close * (1 - _lowerPct/100);

    drawCandleChart('iwmCanvas', d.iwm,
      ...
      [{val:upper,   color:'#3fb950', label:'+'+_upperPct+'%'},
       ...
       {val:iwmOpen, color:'#8b949e', label:'T开$'+iwmOpen.toFixed(2)}],
...
def main():
    ...
    print("运行策略回测（±1）...")
    results_1 = run_backtest(summary_1, call_1_1m, put_1_1m, iwm_1m, iwm_2m, iwm_5m)
    print(f"  ±1 共 {len(results_1)} 个交易日")
    for g, cnt in pd.Series([r['数据粒度'] for r in results_1]).value_counts().items():
        print(f"    {g}: {cnt} 天")

    print("构建日内图表数据...")
    daily_05 = build_daily_charts(results_05, call_05_1m, put_05_1m, iwm_1m, iwm_2m, iwm_5m)
    daily_1  = build_daily_charts(results_1,  call_1_1m,  put_1_1m,  iwm_1m, iwm_2m, iwm_5m)

    print("生成 HTML...")
    html = generate_html(results_05, daily_05, results_1, daily_1)
    with open(OUTPUT_HTML, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"✅ 已生成：{os.path.abspath(OUTPUT_HTML)}")

    total_pnl_05 = sum(r["盈亏"] for r in results_05)

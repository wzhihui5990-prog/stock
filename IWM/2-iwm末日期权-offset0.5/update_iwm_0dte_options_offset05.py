...
行权价规则：Call = ceil(T-1收盘)，Put = floor(T-1收盘)
  例：T-1收盘 200.3 → Call=201, Put=200
  例：T-1收盘 200.0 → Call=200, Put=200
数据来源：Polygon.io 免费 API

输出文件：data/iwm_0dte_options_offset05.xlsx
...
SYMBOL        = "IWM"
# 行权价：Call = math.ceil(T-1收盘)，Put = math.floor(T-1收盘)
# 例如收盘 200.3 → Call=201, Put=200；收盘 200.0 → Call=200, Put=200
RATE_LIMIT    = 3        # Polygon 免费账号：5次/分钟，保守用3
START_DATE    = date(2026, 2, 13)
END_DATE      = None     # None = 拉到今天
OUTPUT_DIR    = "data"
OUTPUT_FILE   = os.path.join(OUTPUT_DIR, "iwm_0dte_options_offset05.xlsx")
...
# IWM 行权价精度为 $0.5，ticker 格式：O:IWM{YYMMDD}{C/P}{STRIKE*1000 zero-padded 8位}
# 示例：IWM 行权价 200.5 → 00200500
# ────────────────────────────────────────────────
...
def process_day(t2_date: str, t2_close: float, t1_date: str) -> dict:
    # Call = 向上取整，Put = 向下取整
    # 例如 200.3 → Call=201, Put=200
    call_strike = math.ceil(t2_close)
    put_strike  = math.floor(t2_close)

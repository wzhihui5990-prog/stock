#!/usr/bin/env python3
"""
将 spy_market_data.xlsx 转换为可视化 HTML 页面（K线 + MA + 成交量 + MACD）
运行（在 1-spy日K 目录下）：python build_spy_market_chart.py
输出：data/spy_market_chart.html
"""
...
INPUT_FILE  = os.path.join(DATA_DIR, "spy_market_data.xlsx")
OUTPUT_FILE = os.path.join(DATA_DIR, "spy_market_chart.html")
...
<header>
  ...
  <div>
    ...
    <div class="meta">数据截至：LAST_DATE &nbsp;·&nbsp; 标普500 ETF</div>
  </div>
...
<div class="tabs">
  ...
  <div class="tab" data-tab="m2">2 分钟</div>
...
const D2MIN  = D2MIN_JSON;
const D5MIN  = D5MIN_JSON;
const DATES1 = DATES1_JSON;
const DATES2 = DATES2_JSON;
...
function switchTab(tab) {
  ...
  const dates   = tab === 'm5' ? DATES5 : tab === 'm2' ? DATES2 : DATES1;
  const dataMap = tab === 'm5' ? D5MIN  : tab === 'm2' ? D2MIN  : D1MIN;
...
def main():
    ...
    df_daily = load_sheet(xls, "SPY_日K")
    df_1min  = load_sheet(xls, "SPY_分时1min")
    df_2min  = load_sheet(xls, "SPY_分时2min")
    df_5min  = load_sheet(xls, "SPY_5min")

    daily_data    = df_to_kdata(df_daily)
    d1min_by_date = intraday_by_date(df_1min)
    d2min_by_date = intraday_by_date(df_2min)
    ...
    dates_2 = sorted(d2min_by_date.keys())
    ...
    html = (HTML_TEMPLATE
        ...
        .replace("D2MIN_JSON",   json.dumps(d2min_by_date, ensure_ascii=False))
        .replace("D5MIN_JSON",   json.dumps(d5min_by_date, ensure_ascii=False))
        .replace("DATES1_JSON",  json.dumps(dates_1,       ensure_ascii=False))
        .replace("DATES2_JSON",  json.dumps(dates_2,       ensure_ascii=False))
    ...
    print(f"   2分钟数据：{len(d2min_by_date)} 天")
    print(f"   5分钟数据：{len(d5min_by_date)} 天")
    print(f"\n   用浏览器打开 spy_market_chart.html 即可查看。")

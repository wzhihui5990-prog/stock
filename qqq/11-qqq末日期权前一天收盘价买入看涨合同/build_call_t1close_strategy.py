# -*- coding: utf-8 -*-
"""
QQQ 末日期权 — 前一天收盘价买入看涨（Call Only）策略回测 + HTML 可视化

策略规则：
  T-1（基准日）收盘：以 Call 期权的 T-1 收盘价 买入 1 张 Call
...
OUTPUT_HTML = os.path.join(os.path.dirname(__file__), "data", "qqq_call_t1close_strategy_report.html")
...
def run_backtest(summary, call_1m, qqq_1m, qqq_2m, qqq_5m):
    ...
    for _, row in summary.iterrows():
        ...
        # 买入价：Call T-1 收盘价
        call_t2_close = row.get("Call_T2收盘")
        if pd.isna(call_t2_close) or float(call_t2_close) <= 0:
            continue
        call_cost = float(call_t2_close)
...
<body>
<div class="header">
  <h1>QQQ 末日期权 — 前一天收盘买入看涨（Call Only）回测分析</h1>
  <div class="sub">0DTE T-1收盘买 Call 策略 &nbsp;·&nbsp; 数据范围：{results3[0]["到期日"]} ~ {results3[-1]["到期日"]} &nbsp;·&nbsp; 共 {total_trades} 个交易日</div>
  <div class="sub-detail">
    <div class="item"><span class="tag green">T-1收盘建仓</span>T-1 收盘以 Call 期权收盘价买入 1 张 Call（行权价 = round(T-1收盘 + N)）</div>
    <div class="item"><span class="tag blue">触发卖出</span>T 日 9:30 起监控 QQQ 相对 T-1 收盘的当日涨跌幅 ≥ +阈值% 或 ≤ −阈值%，立即卖出 Call</div>
...
function selectDay(idx) {
    ...
    <td colspan="15">
      <div class="detail-inner">
        <div class="detail-header">
          ...
          <span style="color:#8b949e">Call: T-1收$${r["Call成本"]}→卖$${r["Call卖出"]} (<span style="color:${parseFloat(callPnl)>=0?'#3fb950':'#f85149'}">$${callPnl}</span>)</span>
          <span style="color:#d29922;font-weight:bold">VIX: ${vixBuy}→${vixSell}</span>
        </div>
        <div class="legend">
          <span><span class="dot" style="background:#58a6ff"></span>T-1收盘买入</span>
  ...
  requestAnimationFrame(() => {
    ...
    drawCandleChart('callCanvas', d.call,
      [{time:'09:30', color:'#58a6ff', label:'T-1收$'+r["Call成本"]}, {time: sellTime, color:'#f0883e', label:'卖$'+r["Call卖出"]}],
      null,
      [['T-1收盘买入','$'+r["Call成本"],'#58a6ff'], ['卖出','$'+r["Call卖出"],'#f0883e'], ['盈亏','$'+callPnl, parseFloat(callPnl)>=0?'#3fb950':'#f85149']]);

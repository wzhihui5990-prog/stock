# -*- coding: utf-8 -*-
"""
QQQ 末日期权 — 前一天收盘价买入看跌（Put Only）策略回测 + HTML 可视化

策略规则：
  T-1（基准日）收盘：以 Put 期权的 T-1 收盘价 买入 1 张 Put
  T（到期日）监控 QQQ 相对 T-1 收盘价的当日涨跌幅：
    若涨幅 ≥ +阈值%，立即卖出 Put
    若跌幅 ≥ −阈值%，立即卖出 Put
...
OUTPUT_HTML = os.path.join(os.path.dirname(__file__), "data", "qqq_put_t1close_strategy_report.html")

UPPER_TRIGGER_PCT = 0.5
LOWER_TRIGGER_PCT = 2.0
COMMISSION   = 1.7
MONITOR_START = "09:30"
MONITOR_END   = "10:30"
...
def run_backtest(summary, put_1m, qqq_1m, qqq_2m, qqq_5m):
    results = []
    ...
    for _, row in summary.iterrows():
        ...
        pnl_pct = (pnl / put_cost * 100) if put_cost > 0 else 0
        ...
        results.append({
            ...
            "QQQ收盘": qqq_close,
            "QQQ涨跌%": qqq_day_pct,
            "Put成本": round(put_cost, 4),
            "Put卖出": round(put_sell, 4),
...
def build_daily_charts(results, put_1m, qqq_1m, qqq_2m, qqq_5m, vix_5min_map=None):
    daily_data = []
    for r in results:
        t1 = r["到期日"]
...
def generate_html(results3, daily3, results4, daily4, vix_daily_data=None):
    ...
    total_cost_sum = sum(r["Put成本"] for r in results)
...
.header h1 {{ font-size: 22px; color: #f85149; }}
...
.section h2 {{ font-size: 16px; color: #c9d1d9; margin-bottom: 10px; border-left: 3px solid #f85149; padding-left: 10px; }}
...
.detail-tr td {{ padding: 0; background: #0a0e17 !important; border-bottom: 2px solid #f85149; cursor: default; }}
...
.ctrl-btn {{ background:#da3633; border:none; border-radius:5px; color:#fff; font-size:12px; padding:5px 14px; cursor:pointer; font-weight:bold; }}
.ctrl-btn:hover {{ background:#f85149; }}
...
.strike-switch button.active {{ background:#da3633; color:#fff; }}
.strike-switch button:hover:not(.active) {{ background:#21262d; color:#e0e0e0; }}
</style>
</head>
<body>
<div class="header">
  <h1>QQQ 末日期权 — 前一天收盘买入看跌（Put Only）回测分析</h1>
  <div class="sub">0DTE T-1收盘买 Put 策略 &nbsp;·&nbsp; 数据范围：{results3[0]["到期日"]} ~ {results3[-1]["到期日"]} &nbsp;·&nbsp; 共 {total_trades} 个交易日</div>
  <div class="sub-detail">
    <div class="item"><span class="tag red">T-1收盘建仓</span>T-1 收盘以 Put 期权收盘价买入 1 张 Put（行权价 = round(T-1收盘 − N)）</div>
    <div class="item"><span class="tag blue">触发卖出</span>T 日 9:30 起监控 QQQ 相对 T-1 收盘的当日涨跌幅 ≥ +阈值% 或 ≤ −阈值%，立即卖出 Put</div>
    <div class="item"><span class="tag green">止损平仓</span>若到平仓时间仍未触发，以当时价格止损卖出</div>
...
function applyThreshold() {
  ...
  for (let i = 0; i < _baseResults.length; i++) {
    ...
    for (const bar of d.qqq) {
      ...
      const pct = (bar.c - t2Close) / t2Close * 100;
    ...
    const pnl = putSell - r['Put成本'] - commissionTotal;
    ...
    r['盈亏'] = +pnl.toFixed(4);
    r['盈亏%'] = r['Put成本'] > 0 ? +(pnl / r['Put成本'] * 100).toFixed(2) : 0;
    cumPnl += pnl;
    newResults.push(r);
  ...
  const totalCost = newResults.reduce((s, r) => s + r['Put成本'] * 100, 0).toFixed(2);
    ...
    <td colspan="15">
      <div class="detail-inner">
        <div class="detail-header">
          <span style="font-size:15px;font-weight:bold;color:#f85149">${r["到期日"]} [${d.granularity}]</span>
          <span style="color:${pnlColor};font-weight:bold">盈亏: $${(r["盈亏"]*100).toFixed(2)}</span>
          <span style="color:#c9d1d9">${trigText}</span>
          <span style="color:#8b949e">Put: T-1收$${r["Put成本"]}→卖$${r["Put卖出"]} (<span style="color:${parseFloat(putPnl)>=0?'#3fb950':'#f85149'}">$${putPnl}</span>)</span>
          <span style="color:#d29922;font-weight:bold">VIX: ${vixBuy}→${vixSell}</span>
        </div>
        <div class="legend">
          <span><span class="dot" style="background:#f85149"></span>T-1收盘买入</span>
          ...
          <span><span class="dot" style="background:#c0392b"></span>-${_lowerPct}%线</span>
        </div>
        <div class="chart-grid2">
          <div class="chart-box">
            <div class="chart-box-title">QQQ 走势（${d.granularity}）— 涨跌基准: T-1收盘</div>
  ...
  requestAnimationFrame(() => {
    ...
    drawCandleChart('qqqCanvas', d.qqq,
      ...
      [{val:upper, color:'#3fb950', label:'+'+_upperPct+'%'}, {val:lower, color:'#f85149', label:'-'+_lowerPct+'%'},
       {val:t2Close, color:'#58a6ff', label:'T-1收$'+t2Close.toFixed(2)}, {val:qqqOpen, color:'#8b949e', label:'T开$'+qqqOpen.toFixed(2)}],
      [['卖出时间', sellTime, '#f0883e']], t2Close);
    drawCandleChart('putCanvas', d.put,
      [{time:'09:30', color:'#f85149', label:'T-1收$'+r["Put成本"]}, {time: sellTime, color:'#f0883e', label:'卖$'+r["Put卖出"]}],
      null,
      [['T-1收盘买入','$'+r["Put成本"],'#f85149'], ['卖出','$'+r["Put卖出"],'#f0883e'], ['盈亏','$'+putPnl, parseFloat(putPnl)>=0?'#3fb950':'#f85149']]);
...
window.addEventListener('resize', () => {
  ...
  if (currentIdx >= 0) { const c1 = document.getElementById('qqqCanvas'); const c2 = document.getElementById('putCanvas'); if (c1) _renderCandle(c1, 'qqqCanvas'); if (c2) _renderCandle(c2, 'putCanvas'); }
});
</script>
</body>
</html>"""
...
def main():
    ...
    print("加载数据...")
    summary3, put3_1m, qqq_1m, qqq_2m, qqq_5m = load_data(OPT_FILE_3)
    summary4, put4_1m, _, _, _ = load_data(opt_file_4)
    ...
    print("运行策略回测（−3 Put）...")
    results3 = run_backtest(summary3, put3_1m, qqq_1m, qqq_2m, qqq_5m)
    print(f"  −3 共 {len(results3)} 个交易日")
    for g, cnt in pd.Series([r['数据粒度'] for r in results3]).value_counts().items():
        print(f"    {g}: {cnt} 天")

    print("运行策略回测（−4 Put）...")
    results4 = run_backtest(summary4, put4_1m, qqq_1m, qqq_2m, qqq_5m)
    ...
    print("构建日内图表数据...")
    daily3 = build_daily_charts(results3, put3_1m, qqq_1m, qqq_2m, qqq_5m, vix_5min_map)
    daily4 = build_daily_charts(results4, put4_1m, qqq_1m, qqq_2m, qqq_5m, vix_5min_map)

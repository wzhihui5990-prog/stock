# -*- coding: utf-8 -*-
"""
QQQ 末日期权 开盘双买策略回测 + HTML 可视化
数据来源：
  - QQQ 股价：1-qqq日K/data/qqq_market_data.xlsx
  - 期权数据：4-qqq末日期权日K-当天开盘上下2和上下3股价的期权合同/data/

策略规则：
  T（到期日）9:30 开盘：以 Call 和 Put 的开盘价同时买入各 1 张
  监控 QQQ 相对开盘价涨跌幅，超过 ±阈值% 立即卖出双腿
...
OUTPUT_HTML = os.path.join(os.path.dirname(__file__), "data", "qqq_straddle_open_strategy_report.html")
...
def run_backtest(summary, call_1m, put_1m, qqq_1m, qqq_2m, qqq_5m):
    ...
    for _, row in summary.iterrows():
        ...
        # QQQ T-1 收盘价（展示用）
        qqq_t2_close = float(row["QQQ_T2收盘"]) if "QQQ_T2收盘" in row.index else qqq_open
        ...
        commission_total = COMMISSION * 4 / 100  # 4张次，转换为权利金单位
        ...
        qqq_day_pct = round((qqq_close - qqq_open) / qqq_open * 100, 2)

        results.append({
            "到期日": t1,
            "基准日": t2,
            "QQQ_T2收盘": round(qqq_t2_close, 2),
            "QQQ开盘": round(qqq_open, 2),
            "QQQ收盘": round(qqq_close, 2),
            "QQQ涨跌%": qqq_day_pct,
            "Call成本": round(call_cost, 4),
            "Put成本": round(put_cost, 4),
            "总成本": round(total_cost, 4),
            "触发": trigger_dir if trigger_dir else "未触发",
            "触发时间": trigger_time if trigger_time else f"{MONITOR_END}止损",
...
def build_daily_charts(results, call_1m, put_1m, qqq_1m, qqq_2m, qqq_5m, vix_5min_map=None):
    ...
    for r in results:
        ...
        daily_data.append({"date": t1, "granularity": r["数据粒度"],
                           "qqq": qqq_arr, "call": call_arr, "put": put_arr, "vix": vix_arr})
    return daily_data
...
<body>
<div class="header">
  <h1>QQQ 末日期权 — 开盘双买（Call + Put）回测分析</h1>
  <div class="sub">0DTE 开盘双买策略 &nbsp;·&nbsp; 数据范围：{results2[0]["到期日"]} ~ {results2[-1]["到期日"]} &nbsp;·&nbsp; 共 {total_trades} 个交易日</div>
  <div class="sub-detail">
    <div class="item"><span class="tag green">开盘建仓</span>T（到期日）9:30 以 Call + Put 开盘价各买入 1 张（行权价 = round(T开盘 ± N)）</div>
    <div class="item"><span class="tag blue">触发卖出</span>9:30 起监控 QQQ 相对开盘价涨幅 ≥ +阈值% 或跌幅 ≥ −阈值%，立即卖出双腿</div>
    <div class="item"><span class="tag red">止损平仓</span>若到平仓时间仍未触发，以当时价格止损卖出</div>
    <div class="item"><span class="tag yellow">手续费</span>买入 Call+Put（2张）+ 卖出 Call+Put（2张）= 4 张次，默认 ${COMMISSION}/张，共 ${round(COMMISSION*4,2)}</div>
    <div class="item"><span class="tag blue">盈亏计算</span>（Call卖出价 + Put卖出价）−（Call买入价 + Put买入价）− 手续费，×100 = 实际美元</div>
  </div>
</div>
<div class="ctrl-bar">
...
<div class="section">
  ...
  <table id="tradeTable">
    <thead>
      <tr>
        <th>到期日</th><th>VIX买</th><th>VIX卖</th><th>粒度</th><th>T-1收盘</th><th>T开盘</th><th>QQQ涨跌%(vs开盘)</th>
...
const MONITOR_END = '""" + MONITOR_END + r"""';
...
function applyThreshold() {
  ...
  if (ct < '09:30' || ct > '15:00') { alert('平仓时间需在 09:30 ~ 15:00 之间'); return; }
  ...
  for (let i = 0; i < _baseResults.length; i++) {
    ...
    for (const bar of d.qqq) {
      ...
      const pct = (bar.c - qqqOpen) / qqqOpen * 100;
...
function _renderCandle(canvas, canvasId) {
  ...
  if (refPrice && refPct !== null) {
    const refCol = refPct >= _upperPct ? '#3fb950' : refPct <= -_lowerPct ? '#f85149' : '#d29922';
  ...
  requestAnimationFrame(() => {
    ...
    drawCandleChart('qqqCanvas', d.qqq,
      ...
      [{val:upper, color:'#3fb950', label:'+'+_upperPct+'%'},
       ...
       {val:qqqOpen, color:'#8b94ae', label:'T开$'+qqqOpen.toFixed(2)}],
...
def main():
    print("加载数据...")
    summary2, call2_1m, put2_1m, qqq_1m, qqq_2m, qqq_5m = load_data(OPT_FILE_2)
    summary3, call3_1m, put3_1m, _, _, _ = load_data(OPT_FILE_3)
    ...
    print("运行策略回测（±2 双买）...")
    results2 = run_backtest(summary2, call2_1m, put2_1m, qqq_1m, qqq_2m, qqq_5m)
    ...
    print("构建日内图表数据...")
    daily2 = build_daily_charts(results2, call2_1m, put2_1m, qqq_1m, qqq_2m, qqq_5m, vix_5min_map)

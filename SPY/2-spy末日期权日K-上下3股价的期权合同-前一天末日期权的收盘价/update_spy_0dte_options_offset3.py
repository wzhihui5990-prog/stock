...
行权价偏移：±3 美元（相对T-1收盘价取整后 ±3）
数据来源：Polygon.io 免费 API

输出文件：data/spy_0dte_options_offset3.xlsx
...
END_DATE      = None
OUTPUT_DIR    = "data"
OUTPUT_FILE   = os.path.join(OUTPUT_DIR, "spy_0dte_options_offset3.xlsx")
...
def get_spy_daily_bars(start, end):
...
def process_day(t2_date, t2_close, t1_date):
    ...
    print(f"  T2={t2_date} SPY收盘=${t2_close}  Call=${call_strike} {call_ticker}  Put=${put_strike} {put_ticker}")
...
def main():
    ...
    spy_days = get_spy_daily_bars(fetch_start, end_date)
    if len(spy_days) < 2:
        print("获取交易日失败，请检查 API Key")
        return
    print(f"  共 {len(spy_days)} 个交易日")

    pairs_to_process = []
    for i in range(len(spy_days) - 1):
        t2      = spy_days[i]
        t1_date = spy_days[i + 1]["date"]

#!/usr/bin/env python3
"""
将 iwm_market_data.xlsx 转换为可视化 HTML 页面（K线 + MA + 成交量 + MACD）
运行（在 IWM/1-iwm日K/ 目录下）：python build_iwm_market_chart.py
...
INPUT_FILE  = os.path.join(DATA_DIR, "iwm_market_data.xlsx")
OUTPUT_FILE = os.path.join(DATA_DIR, "iwm_market_chart.html")
...
def main():
    ...
    df_daily = load_sheet(xls, "IWM_日K")
    df_1min  = load_sheet(xls, "IWM_分时1min")
    df_2min  = load_sheet(xls, "IWM_分时2min")
    df_5min  = load_sheet(xls, "IWM_5min")
    ...
    print(f"\n   用浏览器打开 iwm_market_chart.html 即可查看。")

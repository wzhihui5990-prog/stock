...
END_DATE      = None
OUTPUT_DIR    = "data"
NEW_OUTPUT_FILE = os.path.join(OUTPUT_DIR, "qqq_0dte_options_offset4.xlsx")
OLD_OUTPUT_FILE = os.path.join(OUTPUT_DIR, "QQQ_0DTE_4.xlsx")

# 兼容旧命名文件：如果新文件不存在但旧文件存在，则继续在旧文件上增量更新。
...
def process_day(t2_date, t2_close, t1_date):
    ...
    print(f"  T2={t2_date} QQQ收盘=${t2_close}  Call=${call_strike} {call_ticker}  Put=${put_strike} {put_ticker}")
    ...
    print(f"    Call: T2收盘={'$'+str(call_t2['收盘价']) if call_t2 else '无'}  T1收盘={'$'+str(call_t1['收盘价']) if call_t1 else '无'}  1min={len(call_1m)}条")
    print(f"    Put : T2收盘={'$'+str(put_t2['收盘价'])  if put_t2  else '无'}  T1收盘={'$'+str(put_t1['收盘价'])  if put_t1  else '无'}  1min={len(put_1m)}条")
